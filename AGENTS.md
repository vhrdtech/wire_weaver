...
(the `#[derive_shrink_wrap]` macro — very detailed, read before touching any type definitions), `docs/api/overview.md`
(methods/streams/properties/traits), and `docs/evolution/rules.md` (what changes are wire-compatible). The docs site
source (mkdocs/zensical) lives under `docs/`; prefer it before digging into source, but it unclear, source and comments
in it are authoritative.
