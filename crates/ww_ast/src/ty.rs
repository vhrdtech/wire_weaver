...
impl Type {
    pub(crate) fn from_syn(ty: syn::Type) -> Result<(Self, Vec<SynConversionWarning>), Vec<SynConversionError>> {
        match ty {
            syn::Type::Path(type_path) => {
                if type_path.path.segments.len() == 1 {
                    ...
                    if ident.starts_with('f') {
                        ...
                        Ok((Type::Discrete(TypeDiscrete { is_signed: false, bits }), vec![]))
                    } else if ident.starts_with('i') {
                        let bits: u16 = ident.strip_prefix('i').unwrap().parse().unwrap();
                        Ok((Type::Discrete(TypeDiscrete { is_signed: true, bits }), vec![]))
                    } else if ident == "bool" {
                        Ok((Type::Bool, vec![])) 
                    } else {
