...
///
/// See
pub struct AnyOnStack<const N: usize, T> {
    data: [u8; N],
    used: usize,
    _type: PhantomData<T>,
}

impl<'i, T: SerializeShrinkWrap + DeserializeShrinkWrap<'i>, const N: usize> AnyOnStack<N, T> {
...
#[cfg(test)]
mod tests {
    use crate::any_on_stack::AnyOnStack;
    use hex_literal::hex;

    #[test]
    fn string_array() {
        let value = ["abc", "def", "gh"];
        let value = AnyOnStack::<32, _>::some(value).unwrap();
    ...
    fn byte_arrays() {
        ...
        let value = AnyOnStack::<6, _>::some(([1u8, 2], [3u8, 4, 5, 6])).unwrap();
