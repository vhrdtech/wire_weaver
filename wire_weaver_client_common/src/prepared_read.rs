...
impl<T: DeserializeShrinkWrapOwned + Debug> PreparedRead<T> {
    ...
    /// Send read request, await response (or timeout) and return it.
    ...
    /// Send read request, block the thread until response is received (or timeout) and return it.
    ...
    /// Send read request and return a Promise that can be used to await a result. Useful for immediate mode UI.
